...
import { useMemoDb, useCollection, useFirestore, useUser, deleteDocumentNonBlocking, collection, query, doc, getDoc, collectionGroup, where } from '@/db';
...
export default function SessionsPage() {
  ...
  // Fetch ALL votes once at the page level instead of once per session card
  ...
  // Build a Set of sessionIds where the current user has voted
  const votedSessionIds = useMemo(() => {
    if (!allVotes || !user) return new Set<string>();
    return new Set(
      allVotes
        .filter((v: any) => v.voterParticipantId === user.uid || v.voterParticipantId === (user as any).email)
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {filteredSessions.map((session) => (
            <SessionCard 
              ...
              session={session} 
              isAdmin={isAdmin}
              hasVoted={votedSessionIds.has(session.id)}
...
function SessionCard({ session, isAdmin, hasVoted, onDelete }: { session: any; isAdmin: boolean; hasVoted: boolean; onDelete: (id: string) => void }) {
  const { user } = useUser();
  const firestore = useFirestore();
