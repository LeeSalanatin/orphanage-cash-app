...
import { useMemoDb, useCollection, useUser, useFirestore, collection, query, limit, doc, getDoc, collectionGroup, where } from '@/db';
...
export default function Dashboard() {
  ...
  const userParticipantQuery = useMemoDb(() => {
  ...
  const { data: rawEventsBase, isLoading: eventsLoading } = useCollection(allEventsQuery);
  ...
  const rawEvents = useMemo(() => {
    if (!rawEventsBase || !allGroups) return rawEventsBase || [];
    return rawEventsBase.map((r: any) => {
      let preachingGroupId = r.preachingGroupId;
      if (!preachingGroupId) {
        const group = allGroups.find((g: any) => g.members && g.members[r.participantId]);
        if (group) {
